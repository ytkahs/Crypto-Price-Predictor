...
## Description
This project aim to predict the closing prices of Bitcoin (BTC) and Ethereum (ETH) using a Bidirectional LSTM neural network.
...
## Model Architecture
* **Architecture:** Uses TensorFlow/Keras with a Bidirectional LSTM architecture to learn from time sequences.
...
The model is **good at identifying general market trends** (bull market vs. bear market) but is **not accurate at all for high-frequency trading**. It tends to smooth out extreme spikes and drops. To improve this, we could try adding more external data or adjusting the sequence length for example.
